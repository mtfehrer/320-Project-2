...
SetAssociativeNoAllocation::SetAssociativeNoAllocation()
{
    offsetBits = 5;
    currentTime = 0;
    ...
    for (int i = 0; i < (int)associativitySizes.size(); i++)
    {
        ...
        {
            vector<cacheEntrySANA> v;
            for (int k = 0; k < entriesInEachWay; k++)
            {
                v.push_back(cacheEntrySANA(0, 0));
            }
...
pair<bool, int> SetAssociativeNoAllocation::searchForMatch(int ways, unsigned int cacheIndex, unsigned int tag)
{
...
void SetAssociativeNoAllocation::replaceLRU(int LRUWayIndex, int ways, unsigned int cacheIndex, unsigned int tag)
{
